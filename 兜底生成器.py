...
{"名称":"圆面积示例","领域":"生成","层级":0,"描述":"根据半径求圆面积","输入":[{"名":"参数","类型":"列表[数]"}],"输出":{"类型":"数"},"导出名":"圆面积示例","源码":"# 积木：圆面积示例\n导出 圆面积示例\n段落 圆面积示例 接收 参数：\n    设 半径 为 参数[0]\n    设 圆周率 为 3.14159\n    返回 圆周率 乘 半径 乘 半径\n"}

【范例3·奇偶判断】单值输入用 `接收 数`，用 `数 模 2` 判余，`返回 "奇数"` / `返回 "偶数"`（直引号字符串，不要返回 1/0 或 真/假）：
{"名称":"奇偶示例","领域":"生成","层级":0,"描述":"判断一个整数是奇数还是偶数","输入":[{"名":"数","类型":"数"}],"输出":{"类型":"文本"},"导出名":"奇偶示例","源码":"# 积木：奇偶示例\n导出 奇偶示例\n段落 奇偶示例 接收 数：\n    设 余数 为 数 模 2\n    如果 余数 等于 1：\n        返回 \"奇数\"\n    否则：\n        返回 \"偶数\"\n"}

【范例4·倍数枚举】输入是 `[基数,个数]` 列表时同样用「单入参 参数 + 参数[0]/参数[1]」，循环 `结果.追加(基数 乘 i)` 产出列表：
{"名称":"倍数示例","领域":"生成","层级":0,"描述":"列出某数的前若干个倍数","输入":[{"名":"参数","类型":"列表[数]"}],"输出":{"类型":"列表[数]"},"导出名":"倍数示例","源码":"# 积木：倍数示例\n导出 倍数示例\n段落 倍数示例 接收 参数：\n    设 基数 为 参数[0]\n    设 个数 为 参数[1]\n    设 结果 为 []\n    设 i 为 1\n    当 i 小于等于 个数：\n        结果.追加(基数 乘 i)\n        设 i 为 i 加 1\n    返回 结果\n"}
...
_范例库 = [
    ...
    {"关键词": ["各位数字之和", "各位", "数字和", "逐位"], "块": {
        ...
        "输出": {"类型": "数"}, "导出名": "各位数字之和示例",
        "源码": "# 积木：各位数字之和示例\n导出 各位数字之和示例\n段落 各位数字之和示例 接收 数：\n    设 文本 为 字符串(数)\n    设 和 为 0\n    设 i 为 0\n    当 i 小于 长度(文本)：\n        设 位 为 整数(文本[i])\n        设 和 为 和 加 位\n        设 i 为 i 加 1\n    返回 和\n"}},
    {"关键词": ["平均绝对偏差", "绝对偏差", "偏差"], "块": {
        "名称": "平均绝对偏差示例", "领域": "生成", "层级": 0,
        "描述": "求一组数的平均绝对偏差", "输入": [{"名": "参数", "类型": "列表[数]"}],
        "输出": {"类型": "数"}, "导出名": "平均绝对偏差示例",
        "源码": "# 积木：平均绝对偏差示例\n导出 平均绝对偏差示例\n段落 平均绝对偏差示例 接收 参数：\n    设 序列 为 参数[0]\n    设 n 为 长度(序列)\n    设 均值 为 0\n    设 i 为 0\n    当 i 小于 n：\n        设 均值 为 均值 加 序列[i]\n        设 i 为 i 加 1\n    设 均值 为 均值 除 n\n    设 偏差和 为 0\n    设 j 为 0\n    当 j 小于 n：\n        设 差 为 序列[j] 减 均值\n        如果 差 小于 0：\n            设 差 为 0 减 差\n        设 偏差和 为 偏差和 加 差\n        设 j 为 j 加 1\n    返回 四舍五入(偏差和 除 n, 3)\n"}},
    {"关键词": ["下一闰年", "闰年", "下一个闰年"], "块": {
