# -*- coding: utf-8 -*-
"""段言积木组合总入口 v0.15：需求 + 输入 → 选块 →（契约级接线）→ 校验 → 内联粘合 → 运行。

v0.15 变更（解决词法盲区 + 生成块自动层级）：
  - 默认选块 = embedding 概念图选块（embedding选块.embedding_select）；库外概念→空候选→兜底
  - 新增 LLM/本地校验器（校验器.validate）作为运行前第二道闸，即便误选也能拦
  - 新增 生成/ 积木自动织入 L1+（层级生成.自动织，--自动层级 触发）
  - 选块策略可切换：--关键词（v0 字符重叠）/ --语义（TF-IDF）/ 默认 embedding
...
def _cli(argv=None):
    p = argparse.ArgumentParser(
        description='段言积木组合 v0.15（embedding 选块 + 校验器 + 生成块自动层级）')
