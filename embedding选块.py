...
_概念词典 = {
    ...
    '繁简转换': ['繁体', '简体', '简繁', '繁简'],
    '标准差':   ['标准差'],
    '前缀和':   ['前缀和', '累加序列', '累积和', '累加和'],
    '绝对值':   ['绝对值', 'abs'],
    '日期时间': ['日期', '时间戳', '格式化时间', '年-月-日', '相差多少天', '几天'],
    'JSON':     ['JSON', 'json', '反序列化', '序列化'],
    '正则':     ['正则', 'regex', '正则表达式'],
    '编解码':   ['base64', 'Base64', '编码', '解码', 'urlencode'],
    '合法校验': ['身份证', '校验码', '合法性', '是否合法'],
    '文件IO':   ['CSV', 'csv', '读取文件', '文件内容', '写入文件', '读取本地'],
    'IP地址':   ['IP 地址', 'IP地址', 'ip地址', 'IP', 'ip'],
    'UUID':     ['UUID', 'uuid', 'guid', '唯一标识'],
    '相似度':   ['相似度', '编辑距离', '余弦相似'],
