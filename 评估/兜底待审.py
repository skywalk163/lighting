...
不通过则回滚到 `生成/待审/` 并记入 `待审清单.jsonl`，不污染索引。本脚本是人眼复审入口：

  python 评估/兜底待审.py                 # 列出待审块
  python 评估/兜底待审.py --通过 转二进制   # 人工确认无误后正式入库（写 生成/ + 索引）
  python 评估/兜底待审.py --删除 转二进制   # 确认是坏块，丢弃
  python 评估/兜底待审.py --试运行 转二进制 # 用 组合 跑一次看输出，辅助判断
